 ...
 * @author ab1d_rahman
 */
@WebServlet(urlPatterns = {"/cancelBookingServlet"})
public class cancelBookingServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        bookingInfo bI = new bookingInfo(null, null, null, null, null, bookingID);
        cancelBookingService  CancelBookingService;
        CancelBookingService = new cancelBookingService();
        
        boolean result=false;
        try {
            result = CancelBookingService.deleteBooking(bI, request.getSession());
        } catch (ClassNotFoundException ex) {
            Logger.getLogger(cancelBookingServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
        
        if(result) response.sendRedirect("cancelSuccessful.jsp");
        else response.sendRedirect("cancelUnsuccessful.jsp");
        processRequest(request, response);
    }
